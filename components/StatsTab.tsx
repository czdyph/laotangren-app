...
import { ChevronLeft, ChevronRight, Share, Coffee, Flame, X, Gamepad2 } from "lucide-react";
...
export default function StatsTab(props: StatsTabProps) {
    ...
    if (weekStart === "monday") {
        daysToSubtractForWeek = dayOfW === 0 ? 6 : dayOfW - 1;
    }
    const startOfWeek = new Date(
        currentYear,
        currentMonth,
        currentDay - daysToSubtractForWeek,
    );
    const endOfWeek = new Date(
        currentYear,
        currentMonth,
        currentDay - daysToSubtractForWeek + 6,
    );

    const [gravityMode, setGravityMode] = React.useState(false);
    ...
    const unlabeledBrandName =
        prefLanguage === "English" ? "Unlabeled" : "未标记品牌";
    const otherBrandName = prefLanguage === "English" ? "Others" : "其他";

    const brandMetricMap = statRecords.reduce(
        (acc, record) => {
    ...
    const mergedBrandMetrics =
        otherBrandMetrics.length > 0
            ? [
                ...topBrandMetrics,
                {
    ...
    const donutTotal = mergedBrandMetrics.reduce(
        (sum, item) => sum + item[brandMetricKey],
        0,
    );
    const brandDonutItems = mergedBrandMetrics.map(
        (item): BrandMetricItem => ({
            ...item,
    ...
    const receiptTopBrand = getMostFrequent(
        statRecords.map((r) => r.brand).filter((b) => b),
    );
    const receiptTopTemp = getMostFrequent(
        statRecords.map((r) => r.temperature).filter((t) => t),
    );
    const receiptTopSweet = getMostFrequent(
        statRecords.map((r) => r.sweetness).filter((s) => s),
    );

    const currentCupLimit =
        statPeriod === "week"
            ? parseInt(weeklyCupLimit) || 0
            : statPeriod === "month"
                ? parseInt(monthlyCupLimit) || 0
                : 0;
    const currentBudget =
        statPeriod === "week"
            ? parseFloat(weeklyBudget) || 0
            : statPeriod === "month"
    ...
    const renderDisciplineBar = (
        ...
        if (isDanger)
            statusText = isMoney
                ? `超标 ￥${Math.abs(remaining).toFixed(1)} `
                : `超标 ${Math.abs(remaining)} 杯 `;
        else if (isWarning)
            statusText = isMoney
                ? `仅剩 ￥${remaining.toFixed(1)} `
                : `仅剩 ${remaining} 杯 `;
        else
            statusText = isMoney
                ? `剩余预算 ￥${remaining.toFixed(1)}`
                : `剩余额度 ${remaining} 杯`;
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="px-5 pb-32 pt-[calc(env(safe-area-inset-top)+32px)]"
            >
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center bg-bg-input/60 rounded-full py-1.5 px-1 shadow-sm border border-border-main/50 text-text-muted transition-colors">
                        <button
                            onClick={handlePrevStatPeriod}
                            className="p-1 px-1.5 hover:text-text-main rounded-full transition-colors active:scale-95"
                        >
                ...
                <div className="flex gap-2 mb-6">
                    <button
                        onClick={() => {
                            setStatPeriod("week");
                            setStatAnimationKey((prev) => prev + 1);
                        }}
                    ...
                    {statCups > 0 && (
                        <button
                            onClick={() => setShowStatsPoster(true)}
                    ...
                    <button
                        onClick={() => setShowStatsReceipt(true)}
                ...
                {(() => {
                    ...
                    return (
                        <div className="bg-bg-card rounded-[32px] h-64 w-full relative z-0 mb-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-border-main overflow-hidden flex items-end justify-center pb-2 px-4">
                            {/* 🚀 极其紧凑的极简版物理开关 */}
                            <button
                                onClick={() => {
                                    const nextMode = !gravityMode;
                                    setGravityMode(nextMode);
                                    if (nextMode) triggerHaptic('heavy');
                                    else triggerHaptic('light');
                                }}
                                className={`absolute top-3 right-3 z-[60] flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold tracking-wider backdrop-blur-sm transition-all duration-300 border ${gravityMode
                                    ? 'bg-text-main text-bg-app border-transparent shadow-md'
                                    : 'bg-bg-card/40 text-text-muted border-border-main/30 hover:bg-bg-input hover:text-text-main'
                                    }`}
                            >
                                {gravityMode ? (
                                    <>
                            ...
                            {gravityMode ? (
                                /* 🚀 重力游乐场模式 */
                                <GravityBox records={statRecords} triggerHaptic={triggerHaptic} statPeriod={statPeriod} />
                            ) : (
                                <>
                                    <style
                            ...
                            `,
                                    ...
                                    <div className="flex flex-wrap-reverse justify-center max-w-[95%]">
                                        {statRecords.map((r, i) => {
                                            ...
                                            // 正常模式：5 重叠加滤镜 (极耗 GPU)
                                            const heavyFilter =
                                                "drop-shadow(2px 2px 0 white) drop-shadow(-2px -2px 0 white) drop-shadow(2px -2px 0 white) drop-shadow(-2px 2px 0 white) drop-shadow(0 4px 6px rgba(0,0,0,0.1))";
                                            // 海量模式：1 层微白边 + 1 层轻阴影 (GPU 负载直降 80%)
                                            const lightFilter =
                                                "drop-shadow(0 0 1px rgba(255,255,255,0.8)) drop-shadow(0 2px 4px rgba(0,0,0,0.15))";
                                            const activeFilter = isMassive
                                                ? lightFilter
                                                : heavyFilter;
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            setStatsSelectedRecord(r);
                                                        }}
                                                    >
                                                        {isExportableImageSrc(r.imageUrl) ? (
                                                            // eslint-disable-next-line @next/next/no-img-element
                                                            <motion.img
                                                                layoutId={`drink-img-${r.id}`}
                                                                src={r.imageUrl}
                                                                alt="drink"
                                                                loading="lazy"
                                                                decoding="async"
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    transition={{
                                                        delay:
                                                            i * Math.min(0.05, 0.95 / Math.max(count, 1)),
                                                        type: "spring",
                                                        bounce: 0.5,
                                                    }}
                                    ...
                                    {statRecords.length === 0 && (
                                        <div className="absolute inset-0 flex items-center justify-center text-text-muted text-sm">
                                            {prefLanguage === "English"
                                                ? "No tastings in this period yet"
                                                : "这段时间还没有品鉴"}
                                        </div>
                                    )}
                                </>
                    ...
                    (() => {
                        ...
                        statRecords.forEach((r) =>
                            yearRecordsMap.set(
                                ...
                                (yearRecordsMap.get(`${r.year}-${r.month}-${r.day}`) || 0) + 1,
                            ),
                        );
                        ...
                        for (
                            ...
                            const count =
                                yearRecordsMap.get(
                                    `${currentYear}-${d.getMonth()}-${d.getDate()}`,
                                ) || 0;
                            heatmapDays.push({
                                month: d.getMonth(),
                                day: d.getDate(),
                                count,
                                dateStr: `${currentYear}-${d.getMonth()}-${d.getDate()}`,
                            });
                            if (count > 0) {
                                currentStreak++;
                        ...
                        heatmapDays.forEach((item, index) => {
                            ...
                            if (item && item.day === 1)
                                monthLabels.push({ month: item.month + 1, col: colIndex });
                        });
                        ...
                        return (
                            <div className="bg-bg-card rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] mb-4 w-full">
                                ...
                                <style dangerouslySetInnerHTML={{
                                    __html: `
                                    @keyframes heatMapTileReveal {
                                        0% { opacity: 0; transform: scale(0.3) translateY(10px); filter: brightness(0); }
                                        100% { opacity: 1; transform: scale(1) translateY(0); filter: brightness(1); }
                                    }
                                    .heatmap-tile-animate {
                                        animation: heatMapTileReveal 0.6s cubic-bezier(0.22, 1, 0.36, 1) forwards;
                                        opacity: 0; /* 初始隐藏，等待延迟触发 */
                                    }
                                `}} />
                                <div className="flex justify-between items-center mb-3">
                                    <div>
                                        <span className="text-base text-text-main font-bold block">
                                            {currentYear} 年度热力图
                                        </span>
                                    ...
                                    {maxStreak >= 5 && (
                                        <motion.div
                                            ...
                                            transition={{ delay: 0.8 }} // 🚀 让它等待 0.8 秒后再丝滑出现
                                            onClick={() => {
                                                triggerHaptic("medium");
                                                showToast(
                                                    `经检测，今年你曾连续 ${maxStreak} 天重度堕落，当时是发财了吗？`,
                                                    "info",
                                                );
                                            }}
                                            className="flex items-center gap-1 bg-orange-500/10 text-orange-600 px-2 py-1 rounded-full text-[10px] font-bold cursor-pointer hover:bg-orange-500/20 active:scale-95 transition-all shadow-sm"
                                        >
                                ...
                                <div className="w-full overflow-x-auto custom-scrollbar pb-3 pt-1 -mx-2 px-2">
                                    <div
                                        className="flex text-[9px] text-text-muted font-bold mb-1.5 relative h-3"
                                        style={{
                                            width: `${Math.ceil(heatmapDays.length / 7) * 14}px`,
                                        }}
                                    >
                                    ...
                                    <div className="grid grid-rows-7 grid-flow-col gap-[4px] min-w-max">
                                        {heatmapDays.map((item, index) => {
                                            ...
                                            if (item.count === 1) bgColor = "bg-amber-400";
                                                ...
                                                bgColor = "bg-red-500";
                                                shadowClass =
                                                    "shadow-[0_0_8px_rgba(239,68,68,0.6)] z-10 relative";
                                            }
                                            return (
                                                <div
                                                    // 🌟 核心 1：利用 statAnimationKey 确保每次切换回“年”视图，都会重新触发一遍扫掠动画
                                                    key={`${item.dateStr}-${statAnimationKey}`}
                                                    onClick={() => {
                                                        triggerHaptic("light");
                                                        showToast(
                                                            item.count === 0
                                                                ? `${item.month + 1}月${item.day}日，胰岛很安全，0 杯`
                                                                : `${item.month + 1}月${item.day}日，喝了 ${item.count} 杯，血液纯度已降至糖点。`,
                                                            "info",
                                                        );
                                                    }}
                                                    // 🌟 核心 2：加入我们刚才定义的 heatmap-tile-animate 类名
                                                    className={`heatmap-tile-animate w-[10px] h-[10px] rounded-[2px] cursor-pointer hover:ring-2 ring-border-main hover:scale-125 transition-all duration-200 ${bgColor} ${shadowClass}`}
                                                    style={{
                                                        // 🌟 核心 3：Staggered 延迟算法！
                                                        // 越靠后的格子延迟越大，但为了不让用户等太久，最大延迟锁死在 1.2s 内
                                                        animationDelay: `${Math.min(index * 0.004, 1.2)}s`
                                                    }}
                                                ></div>
                                            );
                                        })}
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div className="bg-bg-card rounded-[24px] p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex flex-col justify-between min-w-0">
                        <div className="min-w-0 w-full">
                            ...
                            <div
                                className={`text-3xl sm:text-4xl font-black font-mono tracking-tighter truncate ${currentCupLimit > 0 && statCups >= currentCupLimit && statPeriod !== "year" ? "text-red-500" : ""}`}
                                style={
                                    currentCupLimit > 0 &&
                                        statCups >= currentCupLimit &&
                                        statPeriod !== "year"
                                        ? {}
                                        : { color: themeAccent }
                                }
                    ...
                    <div className="bg-bg-card rounded-[24px] p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] flex flex-col justify-between min-w-0">
                        <div className="min-w-0 w-full">
                            ...
                            <div
                                className={`text-3xl sm:text-4xl font-black font-mono tracking-tighter truncate ${currentBudget > 0 && statCost >= currentBudget && statPeriod !== "year" ? "text-red-500" : ""}`}
                                style={
                                    currentBudget > 0 &&
                                        statCost >= currentBudget &&
                                        statPeriod !== "year"
                                        ? {}
                                        : { color: themeAccent }
                                }
                            >
                                {/* 🚀 注入滚动组件，开启金额模式 */}
                                <NumberTicker value={statCost} isCurrency={true} />
                            </div>
                        </div>
                    ...
                    <div className="bg-bg-card rounded-[24px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] col-span-2">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-base text-text-main font-bold">
                                {prefLanguage === "English" ? "Brand Mix" : "品牌占比"}
                            </span>
                                    ...
                                    {brandDonutItems.map((item) => (
                                        ...
                                        >
                                            ...
                                            <div className="text-text-muted font-medium shrink-0">
                                                {brandDonutMode === "cups"
                                                    ? `${item.cups}${prefLanguage === "English" ? "" : "杯"}`
                                                    : `￥${item.cost.toFixed(1)}`}{" "}
                    ...
                    <div className="bg-bg-card rounded-[24px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] col-span-2">
                        <span className="text-base text-text-main font-bold block mb-4">
                            {chartTitle}
                        </span>
                        <div className="flex justify-between items-end h-[100px] gap-1 sm:gap-2 mt-2">
                            {chartValues.map((val, idx) => (
                                ...
                                >
                                    <span className="text-text-muted text-xs font-bold min-h-[16px]">
                                        {val > 0 ? val : ""}
                                    </span>
                                    <motion.div
                                        initial={{ height: 0 }}
                                        animate={{
                                            height:
                                                val > 0
                                                    ? `${Math.max((val / maxChartVal) * 100, 8)}%`
                                                    : "4px",
                                        }}
                                    ...
                                    />
                                    <span className="text-[10px] text-text-muted font-medium whitespace-nowrap mt-1 flex-shrink-0">
                                        {chartLabels[idx]}
                                    </span>
                                </div>
                            ))}
            ...
            <AnimatePresence>
                {showStatsReceipt && (
                    ...
                    >
                        {/* 🚀 核心修复：把缩放动画合并到这里，并加入 will-change-transform 开启 GPU 加速 */}
                        <motion.div
                            ...
                            onDragEnd={(e, info) => {
                                if (info.offset.x > 100 || info.velocity.x > 500)
                                    setShowStatsReceipt(false);
                            }}
                            className="min-h-full flex flex-col items-center justify-center py-12 px-4 w-full will-change-transform"
                        >
                            {/* 🚀 去掉 motion.div，改为普通 div，避免嵌套计算 */}
                            <div
                                ref={receiptCardRef}
                            ...
                            >
                                    ...
                                    <div className="w-16 h-20 mr-1">
                                        {statRecords.length > 0 &&
                                            isExportableImageSrc(
                                                statRecords[statRecords.length - 1].imageUrl,
                                            ) ? (
                                            // eslint-disable-next-line @next/next/no-img-element
                                            <img
                                                src={statRecords[statRecords.length - 1].imageUrl}
                                            ...
                                            <Coffee
                                                size={56}
                                                className="text-[#102a4a] mt-1 ml-1 drop-shadow-xl"
                                                strokeWidth={1.8}
                                            />
                                        )}
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-[12px] font-medium text-gray-800 mb-4">
                                    <div>
                                        <div className="text-gray-400 tracking-wider text-[9px] mb-0.5">
                                            {prefLanguage === "English" ? "PERIOD" : "统计周期"}
                                        </div>
                                        <div className="font-bold text-[13px]">{statDateLabel}</div>
                                    </div>
                                    <div>
                                        <div className="text-gray-400 tracking-wider text-[9px] mb-0.5">
                                            {prefLanguage === "English" ? "TOTAL CUPS" : "杯数（总）"}
                                        </div>
                                        <div className="font-bold text-[13px]">
                                            {statCups} {prefLanguage === "English" ? "Cups" : "杯"}
                                        </div>
                                    </div>
                                    {statCups > 0 && (
                                        <>
                                            <div>
                                                ...
                                                <div className="font-bold text-[13px] truncate pr-2">
                                                    {receiptTopBrand}
                                                </div>
                                            </div>
                                            <div>
                                                ...
                                                <div className="font-bold text-[13px] truncate">
                                                    {receiptTopTemp} · {receiptTopSweet}
                                                </div>
                                            </div>
                                        </>
                                    )}
                                    <div className="col-span-2 pt-2 mt-1 border-t border-dashed border-gray-200 flex justify-between items-end">
                                        <span className="text-gray-400 tracking-wider text-[11px] mb-1">
                                            {prefLanguage === "English" ? "TOTAL COST" : "消费金额"}
                                        </span>
                                        <span className="font-black text-[32px] leading-none text-[#102a4a]">
                                            ￥{parseFloat(statCost.toFixed(2))}
                                        </span>
                                    </div>
                                </div>
                                {statCups > 0 && (
                                    <div className="mb-3 border-y border-dashed border-gray-300 py-2">
                                        <div className="flex justify-between text-[9px] text-gray-400 font-bold mb-1.5 tracking-widest">
                                            ...
                                            <span>
                                                {prefLanguage === "English" ? "AMOUNT" : "金额"}
                                            </span>
                                        </div>
                                        <div className="space-y-1.5">
                                            {mergedBrandMetrics.map((item, i) => (
                                                <div
                                                    key={i}
                                                    className="flex justify-between items-center text-[11px] font-medium text-gray-800"
                                                >
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="font-bold text-[12px] text-[#102a4a]">
                                                            {item.brand}
                                                        </span>
                                                    ...
                                                    <div className="font-black text-[13px] text-[#102a4a]">
                                                        ￥{item.cost.toFixed(1)}
                                                    </div>
                                                </div>
                                            ))}
                                ...
                                <div className="flex justify-center h-8 opacity-80 mb-1">
                                    {Array.from({ length: 40 }).map((_, i) => (
                                        <div
                                            key={i}
                                            className={`h-full bg-black mx-[0.5px] ${i % 3 === 0 ? "w-1" : i % 5 === 0 ? "w-1.5" : "w-[2px]"}`}
                                        ></div>
                                    ))}
                                </div>
                                <div className="absolute -bottom-[6px] left-0 right-0 h-3 flex justify-around px-1 overflow-hidden pointer-events-none">
                                    {Array.from({ length: 30 }).map((_, i) => (
                                        <div
                                            key={`dot-${i}`}
                                            className="w-[8px] h-[8px] rounded-full bg-[#1a1a1e]"
                                        ></div>
                                    ))}
                                </div>
                            </div>

                            {/* 🚀 去掉按钮的 motion 动画，直接跟随父容器一并滑入 */}
                            <div className="flex gap-4 mt-8 w-full max-w-[360px]">
                                <button
                                    onClick={() => handleSaveReceipt(receiptCardRef)}
                                    className="flex-1 py-3.5 rounded-full border border-white/80 text-white font-bold bg-transparent tracking-widest text-sm hover:bg-white/10 transition-colors shadow-[inset_0_0_0_1px_rgba(255,255,255,0.15)]"
                                >
                                    {prefLanguage === "English" ? "SAVE" : "保存小票"}
                                </button>
                                <button
                                    onClick={() => handleShareReceipt(receiptCardRef)}
                                    className="flex-1 py-3.5 rounded-full bg-white text-black font-bold tracking-widest text-sm hover:bg-gray-100 transition-colors shadow-[0_10px_24px_rgba(255,255,255,0.25)]"
                                >
                                    {prefLanguage === "English" ? "SHARE" : "分享"}
                                </button>
                            </div>
                            <button
                                onClick={() => setShowStatsReceipt(false)}
            ...
            <AnimatePresence>
                {showStatsPoster && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[999] bg-black/80 overflow-y-auto custom-scrollbar backdrop-blur-xl"
                    >
                        {/* 🚀 核心修复：合并动画，加入 will-change-transform */}
                        <motion.div
                            initial={{ opacity: 0, x: 50, scale: 0.95 }}
                            animate={{ opacity: 1, x: 0, scale: 1 }}
                            exit={{ opacity: 0, x: "100%", scale: 0.95 }}
                            ...
                            onDragEnd={(e, info) => {
                                if (info.offset.x > 100 || info.velocity.x > 500)
                                    setShowStatsPoster(false);
                            }}
                            className="min-h-full flex flex-col items-center justify-center py-12 px-4 w-full will-change-transform"
                        >
                            {/* 🚀 加上 3D 陀螺仪外壳 */}
                            <GyroCard className="w-full max-w-[360px]">
                                <div
                                    ref={posterRef}
                                    className="w-full h-full rounded-3xl relative flex flex-col p-8 text-white shadow-[0_20px_80px_rgba(0,0,0,0.5)] overflow-hidden"
                                    style={{
                                        background:
                                            "linear-gradient(135deg, #111827 0%, #312e81 50%, #4c1d95 100%)",
                                    }}
                                >
                            ...
                            {/* 🚀 去掉按钮的 motion，转为普通 html 标签 */}
                            <div className="flex gap-4 mt-8 w-full max-w-[360px]">
                                <button
                                    onClick={() => handleSaveReceipt(posterRef)}
                                    className="flex-1 py-3.5 rounded-full border border-white/30 text-white font-bold bg-white/10 backdrop-blur-md tracking-widest text-sm hover:bg-white/20 transition-colors"
                                >
                                    {prefLanguage === "English" ? "SAVE" : "保存海报"}
                                </button>
                                <button
                                    onClick={() => handleShareReceipt(posterRef)}
                                    className="flex-1 py-3.5 rounded-full bg-white text-black font-bold tracking-widest text-sm hover:bg-gray-100 transition-colors shadow-lg"
                                >
                                    {prefLanguage === "English" ? "SHARE" : "分享海报"}
                                </button>
                            </div>
                            <button
                                onClick={() => setShowStatsPoster(false)}
                                className="mt-8 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center font-bold text-xl pb-1 hover:bg-white/20 transition-colors border border-white/20 shrink-0"
                            >
                                <X size={22} strokeWidth={2.5} />
                            </button>
                        </motion.div>
            ...
            <AnimatePresence>
                {statsSelectedRecord && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6"
                    >
                        <motion.div
                            initial={{ scale: 0.8, y: 20, x: 50 }}
                            ...
                            onDragEnd={(e, info) => {
                                if (info.offset.x > 100 || info.velocity.x > 500)
                                    setStatsSelectedRecord(null);
                            }}
                            onClick={(e) => e.stopPropagation()}
                            // 🚀 核心：在这里最后面补上 will-change-transform
                            className="w-full max-w-[300px] bg-bg-card rounded-[28px] p-6 relative shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col items-center border border-border-main/50 will-change-transform"
                        >
                            <button
                                onClick={() => setStatsSelectedRecord(null)}
                            ...
                            <div className="text-[10px] font-bold tracking-widest text-text-muted mb-4 theme-bg/10 px-3 py-1 rounded-full uppercase">
                                {statsSelectedRecord.year} /{" "}
                                {String(statsSelectedRecord.month + 1).padStart(2, "0")} /{" "}
                                {String(statsSelectedRecord.day).padStart(2, "0")}
                            </div>
                            <div className="w-28 h-36 flex items-center justify-center mb-5 relative">
                                {isExportableImageSrc(statsSelectedRecord.imageUrl) ? (
                                    <>
                                        {/* 🚀 Apple Music 级投影：直接拿原图拉高饱和度后做重度模糊 */}
                                        <motion.img
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 0.6 }}
                                            transition={{ duration: 0.5, delay: 0.1 }}
                                            src={statsSelectedRecord.imageUrl}
                                            alt="glow"
                                            className="absolute inset-0 w-full h-full object-contain scale-[1.3] blur-[24px] saturate-[250%] z-0 pointer-events-none"
                                        />

                                        {/* 奶茶本体 */}
                                        <motion.img
                                            layoutId={`drink-img-${statsSelectedRecord.id}`}
                                            src={statsSelectedRecord.imageUrl}
                                            alt="drink"
                                            className="w-full h-full object-contain filter drop-shadow-xl scale-110 relative z-10"
                                        />
                                    </>
                                ) : (
                                    <div className="w-full h-full bg-bg-input/50 rounded-2xl flex items-center justify-center -rotate-2 relative z-10">
                            ...
                            <div className="text-center w-full">
                                {statsSelectedRecord.brand && (
                                    <div className="text-[11px] font-bold theme-text mb-1.5">
                                        {statsSelectedRecord.brand}
                                    </div>
                                )}
                                ...
                                <div className="flex flex-wrap justify-center gap-1.5 mb-5">
                                    {[
                                        statsSelectedRecord.size,
                                        statsSelectedRecord.temperature,
                                        statsSelectedRecord.sweetness,
                                    ]
                                        .filter(Boolean)
                                        .map((tag, idx) => (
                                            <span
                                                key={idx}
                                                className="text-[11px] bg-bg-input text-text-muted px-2.5 py-1 rounded-md font-medium"
                                            >
                                                {tag}
                                            </span>
                                        ))}
